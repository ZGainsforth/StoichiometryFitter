...
def AnalyzePhaseWeb(AtPct=None, WtPct=None, OxWtPct=None, OByStoich=None):
    ...
    GEMSAtPct, GEMSAtPctSD, plot1 = GEMSPlotWeb(AtPct, ProtosolarToSi)

    #PrintTernary(AtPct, GEMSAtPct, GEMSAtPctSD)

    Chondricity, ElementSigma, GEMSChiSqRed, DOF, plot2 = GEMSChiPlotWeb(AtPct, Protosolar)
...
def GEMSChiPlotWeb(AtPct, Protosolar):
    ...
    # Plotly
    IncludedZ = np.where(~np.isnan(Chondricity))[0]+1
    TickLabels = [El for Z, El in enumerate(pb.ElementalSymbols) if Z in IncludedZ]
    TickInds = list(range(len(TickLabels)))

    fig = make_subplots()

    # Scatter plot for Chondricity
    trace1 = go.Scatter(
        x=TickInds,
        y=Chondricity[IncludedZ-1],
        mode='markers',
        marker=dict(
            size=15,
            color='red',
    ...
    # Scatter plot for Elemental sigmas from mean
    trace2 = go.Scatter(
        x=TickInds,
        y=ElementSigma[IncludedZ-1],
        mode='markers',
        marker=dict(
            size=15,
            color='blue',
        ...
        name='Elemental sigmas from mean'
    )

    # Line at Chondritic/GEMSitic
    trace3 = go.Scatter(
        x=[0, 92],
        y=[0, 0],
        mode='lines',
    ...
    fig.update_layout(
        yaxis_title='Chondricity:\nlog$_{10}$(At%/Chondritic)\nVariation from GEMS ($\sigma$)',
        autosize=True,
        legend=dict(
            orientation='h',
            yanchor='bottom',
    ...
    return Chondricity, ElementSigma, GEMSChiSqRed, DOF, fig
...
def GEMSPlotWeb(AtPct, ProtosolarToSi):
    ### Draw a plot comparing this spectrum normalized to CI and plotted against GEMS compositions.
    # First we have mean and standard deviation values for GEMS compositions.
    GEMSAtPct, GEMSAtPctSD, RefStr = GetGEMSStandardVals(pb.U-1)
    # Make these Si normalized.
    ...
    fig.update_layout(
        yaxis=dict(
            type='log',
            range=[np.log10(3e-2), np.log10(30)]
        ),
        yaxis_title='Element/Si/chondritic, At%',
        autosize=True,
        legend=dict(
            orientation='h',
            yanchor='bottom',
    ...
    return GEMSAtPct, GEMSAtPctSD, fig
