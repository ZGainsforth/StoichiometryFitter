...
def login():
    if request.method == "POST":
        ...
        if request.form['GO'] == "Go!":
            ...
            if Fig1 and Fig2:
                return render_template('result.html', Result = FinalReport, fig1 = figs[0], fig2 = figs[1])
            else:
                return render_template('result.html', Result = FinalReport, show_api_input=session.get('show_api_input', False))
    else:
